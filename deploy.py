...
def text_detector(img_path=None, vid_path=None,vid_out = None):

    print(f"[INFO] Loading model... ")
    ## loading the custom trained model
    # model =  torch.hub.load('ultralytics/yolov5', 'custom', path='last.pt',force_reload=True) ## if you want to download the git repo and then run the detection
    model =  torch.hub.load('./yolov5', 'custom', source ='local', path='anzo.pt',force_reload=True) ### The repo is stored locally
    ...
    if vid_path !=None:
        ...
        # cv2.namedWindow("Live View", cv2.WINDOW_FULLSCREEN)
        while True:
